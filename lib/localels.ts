...
const COMBINER_SLUG: Record<Language, string> = {
  pt: 'combinador-de-nomes',
  en: 'name-combiner',
  es: 'combinador-de-nombres',
};
...
const COMBINER_LABEL: Record<Language, string> = {
  pt: 'Combinador',
  en: 'Combiner',
  es: 'Combinador',
};
...
const COMBINER_SLUG_TO_LOCALE = Object.fromEntries(
  Object.entries(COMBINER_SLUG).map(([locale, slug]) => [slug, locale]),
) as Record<string, Language>;
...
/** Locale-correct links to the core tools. */
export function toolLinks(language: Language): { generator: ToolLink; family: ToolLink; combiner: ToolLink } {
  return {
    ...
    combiner: { href: `/${language}/${COMBINER_SLUG[language]}`, label: COMBINER_LABEL[language] },
  };
}
...
export function localizedPathForLocale(pathname: string | null | undefined, targetLocale: Language): string {
  ...
  if (pageSlug in COMBINER_SLUG_TO_LOCALE) {
    return toolLinks(targetLocale).combiner.href;
  }
